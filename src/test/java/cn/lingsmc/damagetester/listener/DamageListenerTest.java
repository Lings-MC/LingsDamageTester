...
class DamageListenerTest {
    ...
    private MockedStatic<DamageTester> pluginStatic;
    ...
    void setUp() {
        ...
        pluginStatic = mockStatic(DamageTester.class);

        DamageTester plugin = mock(DamageTester.class);
        FileConfiguration config = mock(FileConfiguration.class);
        when(config.getBoolean(ConfigConstants.SHOW_CAUSE, true)).thenReturn(true);
        when(plugin.getConfig()).thenReturn(config);
        pluginStatic.when(DamageTester::getInstance).thenReturn(plugin);
